  ...
  return (
    <section id="services" className="bg-white pt-12 pb-24 scroll-mt-24 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] xl:max-w-[1600px] 2xl:max-w-[1920px] mx-auto px-6 lg:px-16 relative z-10">
        ...
        {/* 🔥 FIXED: Removed md:items-start and md:mx-0 so it stays perfectly centered everywhere */}
        <header className="mb-10 md:mb-14 2xl:mb-16 max-w-3xl 2xl:max-w-none flex flex-col items-center mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold uppercase tracking-widest mb-5 md:mb-6 shadow-sm">
            {content.badge}
          </div>
          
          {/* 🔥 FIXED: Grouped the gradient as an inline-block */}
          <h2 className="text-center text-4xl md:text-5xl lg:text-6xl 2xl:text-[4.5rem] font-extrabold text-slate-900 mb-0 md:mb-6 tracking-tight leading-[1.1] [text-wrap:balance]">
            {content.headingMain}{' '}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 2xl:gap-10 group/bento items-stretch">
...
const Card = ({ title, description, icon }: { title: string, description: string, icon: React.ReactNode }) => (
  <article className="group relative bg-slate-50 hover:bg-white border border-slate-200 hover:border-amber-300 rounded-[2rem] p-6 md:p-8 transition-all duration-500 overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-amber-500/10 flex flex-col cursor-pointer group-hover/bento:opacity-60 group-hover/bento:blur-[2px] group-hover/bento:scale-[0.98] hover:!opacity-100 hover:!blur-none hover:!scale-100 hover:z-10 h-full">
    ...
    <div className="relative z-20 flex flex-col h-full">
      ...
      <h3 className="text-xl md:text-2xl 2xl:text-3xl font-bold text-slate-900 mb-3 md:mb-4 md:group-hover:text-amber-600 transition-colors duration-300 tracking-tight">
        {title}
      </h3>
      
      <p className="text-sm md:text-base 2xl:text-lg text-slate-600 leading-relaxed">
        {description}
      </p>
    </div>
