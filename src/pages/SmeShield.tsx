      ...
      <div className="max-w-[1440px] xl:max-w-[1600px] 2xl:max-w-[1920px] mx-auto px-6 lg:px-16 relative z-10 w-full">
        ...
        <div className="max-w-4xl xl:max-w-5xl 2xl:max-w-none mb-12 2xl:mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 border border-amber-200 text-amber-800 text-xs 2xl:text-sm font-black uppercase tracking-widest mb-6 shadow-sm">
            <Zap size={14} className="2xl:w-4 2xl:h-4" /> {content.badge}
          </div>
          ...
          <div className="absolute bottom-0 translate-y-1/2 w-[95%] sm:w-[90%] lg:w-[80%] bg-white rounded-2xl md:rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-stone-200 p-4 sm:p-6 lg:p-8 2xl:p-12 z-20 overflow-hidden">
            <div className="grid grid-cols-3 gap-2 sm:gap-6 2xl:gap-10 divide-x divide-stone-200 text-center items-center">
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm 2xl:text-base font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 2xl:mb-2 truncate">{content.turnaroundLabel}</div>
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-black text-slate-900 whitespace-nowrap">{content.turnaroundValue}</div>
              </div>
              <div className="px-1 sm:px-4 md:px-0">
              ...
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm 2xl:text-base font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 2xl:mb-2 truncate">{content.downtimeLabel}</div>
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-black text-emerald-500 whitespace-nowrap">{content.downtimeValue}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 2xl:gap-12">
          ...
          <div className="lg:col-span-7 bg-white border border-stone-200 rounded-[2rem] p-8 lg:p-10 2xl:p-16 shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col justify-center">
            ...
            <p className="text-slate-600 text-base lg:text-lg 2xl:text-xl leading-relaxed font-medium mb-6 lg:mb-8">
              We instantly initiated our <strong className="text-slate-900">{content.approach.healthCheckExpress}</strong> protocol. Our response team was active within 60 minutes.
            </p>
            <ul className="space-y-4 2xl:space-y-6">
              {content.approach.items.map((item, index) => (
                <li key={index} className="flex items-start gap-4 2xl:gap-5">
                  <div className="mt-1 bg-amber-100 p-1.5 2xl:p-2 rounded-md text-amber-700">
                    {index === 0 ? <Clock size={16} className="2xl:w-5 2xl:h-5" strokeWidth={3} /> : <Activity size={16} className="2xl:w-5 2xl:h-5" strokeWidth={3} />}
                  </div>
                  {/* 🔥 UNIFIED BULLET POINTS */}
                  <p className="text-slate-600 font-medium text-base lg:text-lg 2xl:text-xl"><strong className="text-slate-900">{item.timeRange}:</strong> {item.description}</p>
                </li>
              ))}
            </ul>
          </div>
