          ...
          <div className="absolute bottom-0 translate-y-1/2 w-[95%] sm:w-[90%] lg:w-[80%] bg-white rounded-2xl md:rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-stone-200 p-4 sm:p-6 lg:p-8 z-20 overflow-hidden">
            <div className="grid grid-cols-3 gap-2 sm:gap-6 divide-x divide-stone-200 text-center items-center">
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 truncate">{content.turnaroundLabel}</div>
                <div className="text-base sm:text-2xl md:text-3xl lg:text-4xl font-black text-slate-900 whitespace-nowrap">{content.turnaroundValue}</div>
              </div>
              <div className="px-1 sm:px-4 md:px-0">
              ...
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 truncate">{content.downtimeLabel}</div>
                <div className="text-base sm:text-2xl md:text-3xl lg:text-4xl font-black text-emerald-500 whitespace-nowrap">{content.downtimeValue}</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <ul className="space-y-4">
              {content.approach.items.map((item, index) => (
                <li key={index} className="flex items-start gap-4">
                  <div className="mt-1 bg-amber-100 p-1.5 rounded-md text-amber-700">
                    {index === 0 ? <Clock size={16} strokeWidth={3} /> : <Activity size={16} strokeWidth={3} />}
                  </div>
                  <p className="text-slate-600 font-medium"><strong className="text-slate-900">{item.timeRange}:</strong> {item.description}</p>
                </li>
              ))}
            </ul>
          </div>
