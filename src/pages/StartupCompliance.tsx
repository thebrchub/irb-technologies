        ...
        >
          <div className="grid grid-cols-3 gap-0 md:gap-8 divide-x divide-stone-200 text-center items-center">
            <div className="px-1 md:px-2">
              <div className="text-[9px] md:text-xs 2xl:text-sm font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2 truncate">{content.timelineLabel}</div>
              <div className="text-lg md:text-3xl lg:text-4xl 2xl:text-6xl font-black text-slate-900">{content.timelineValue}</div>
            </div>
            <div className="px-1 md:px-2">
              <div className="text-[9px] md:text-xs 2xl:text-sm font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2 truncate">{content.frameworksLabel}</div>
              <div className="text-lg md:text-3xl lg:text-4xl 2xl:text-6xl font-black text-slate-900">{content.frameworksValue}</div>
            </div>
            <div className="px-1 md:px-2">
              <div className="text-[9px] md:text-xs 2xl:text-sm font-bold text-slate-400 uppercase tracking-widest mb-1 md:mb-2 truncate">{content.pipelineLabel}</div>
              <div className="text-lg md:text-3xl lg:text-4xl 2xl:text-6xl font-black text-emerald-500">{content.pipelineValue}</div>
            </div>
          </div>
        </motion.div>
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-5 lg:mb-6 relative z-10">
              <div className="w-12 h-12 2xl:w-14 2xl:h-14 bg-red-500/20 border border-red-500/30 text-red-400 rounded-xl flex items-center justify-center shrink-0">
                <FileCheck size={24} className="2xl:w-7 2xl:h-7" />
              </div>
              ...
              {content.approach.items.map((item, index) => (
                <li key={index} className="flex items-start gap-3 2xl:gap-4">
                  <div className="mt-1 bg-amber-100 p-1.5 rounded-md text-amber-700 shrink-0">
                    {index === 0 ? <Map size={14} className="2xl:w-4 2xl:h-4" strokeWidth={3} /> : <CheckCircle size={14} className="2xl:w-4 2xl:h-4" strokeWidth={3} />}
                  </div>
                  <p className="text-slate-600 text-base md:text-lg leading-relaxed m-0">
                    <strong className="text-slate-900">{item.label}:</strong> {item.description}
                  </p>
                </li>
              ))}
            </ul>
