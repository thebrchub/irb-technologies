...
const StartupCompliance = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] pt-24 md:pt-32 pb-24 md:pb-32 font-sans selection:bg-amber-200 selection:text-amber-900 relative overflow-hidden">
      ...
      {/* 🔥 FIXED: Expanded max-width to 1920px */}
      <div className="max-w-[1440px] xl:max-w-[1600px] 2xl:max-w-[1920px] mx-auto px-6 lg:px-16 relative z-10 w-full">
        ...
        <div className="max-w-4xl xl:max-w-5xl 2xl:max-w-none mb-12 2xl:mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 border border-amber-200 text-amber-800 text-xs 2xl:text-sm font-black uppercase tracking-widest mb-6 shadow-sm">
            <FileCheck size={14} className="2xl:w-4 2xl:h-4" /> {content.badge}
          </div>
          
          {/* 🔥 FIXED: Scaled for 2xl, protected for laptops */}
          ...
          {/* 🔥 FIXED: Single line description on 2xl */}
          <p className="text-lg md:text-xl 2xl:text-2xl 2xl:whitespace-nowrap 2xl:max-w-none text-slate-600 leading-relaxed font-medium max-w-4xl">
            {content.description}
          </p>
        </div>

        <div className="relative w-full mb-20 sm:mb-24 md:mb-28 lg:mb-32 2xl:mb-48 flex flex-col items-center">
          {/* 🔥 FIXED: Image height scales safely up to 2xl */}
          ...
          <div className="absolute bottom-0 translate-y-1/2 w-[95%] sm:w-[90%] lg:w-[80%] bg-white rounded-2xl md:rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-stone-200 p-4 sm:p-6 lg:p-8 2xl:p-12 z-20 overflow-hidden">
            <div className="grid grid-cols-3 gap-2 sm:gap-6 2xl:gap-10 divide-x divide-stone-200 text-center items-center">
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm 2xl:text-base font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 2xl:mb-2 truncate">{content.timelineLabel}</div>
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-black text-slate-900 whitespace-nowrap">{content.timelineValue}</div>
              </div>
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm 2xl:text-base font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 2xl:mb-2 truncate">{content.frameworksLabel}</div>
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-black text-slate-900 whitespace-nowrap">{content.frameworksValue}</div>
              </div>
              <div className="px-1 sm:px-4 md:px-0">
                <div className="text-[9px] sm:text-xs md:text-sm 2xl:text-base font-bold text-slate-400 uppercase tracking-wide md:tracking-widest mb-0.5 md:mb-1 2xl:mb-2 truncate">{content.pipelineLabel}</div>
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-5xl font-black text-emerald-500 whitespace-nowrap">{content.pipelineValue}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 2xl:gap-12">
          ...
          <div className="lg:col-span-5 bg-slate-900 text-white rounded-[2rem] p-8 lg:p-10 2xl:p-16 shadow-xl flex flex-col justify-center relative overflow-hidden group">
            ...
            <div className="w-14 h-14 2xl:w-16 2xl:h-16 bg-red-500/20 border border-red-500/30 text-red-400 rounded-2xl flex items-center justify-center mb-6 lg:mb-8">
              <FileCheck size={28} className="2xl:w-8 2xl:h-8" />
            </div>
            {/* 🔥 UNIFIED TITLE */}
            ...
            <ul className="space-y-4 2xl:space-y-6">
              {content.approach.items.map((item, index) => (
                <li key={index} className="flex items-start gap-4 2xl:gap-5">
                  <div className="mt-1 bg-amber-100 p-1.5 2xl:p-2 rounded-md text-amber-700">{index === 0 ? <Map size={16} className="2xl:w-5 2xl:h-5" strokeWidth={3} /> : <CheckCircle size={16} className="2xl:w-5 2xl:h-5" strokeWidth={3} />}</div>
